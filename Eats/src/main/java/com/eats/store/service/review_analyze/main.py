...
# Windows에서 발생하는 비동기 이벤트 루프 오류 해결
...
def main():
    try:
        # CSV 파일 로드
        df = pd.read_csv('sample_reviews3.csv', encoding='utf-8').head(20)
        print(f"리뷰갯수: {len(df)}")
    except Exception as e:
    ...
    try:
        ...
        # 분석 결과 출력
        print("\n분석 결과:")
        for idx, result in enumerate(results.get('data', []), 1):
            print(f"\n=== 리뷰 {idx} 분석 결과 ===")
            
            print("\n[분위기]")
            print("긍정:", ", ".join(result['분위기']['긍정']) if result['분위기']['긍정'] else "없음")
            print("부정:", ", ".join(result['분위기']['부정']) if result['분위기']['부정'] else "없음")
            
            print("\n[서비스]")
            print("긍정:", ", ".join(result['서비스']['긍정']) if result['서비스']['긍정'] else "없음")
            print("부정:", ", ".join(result['서비스']['부정']) if result['서비스']['부정'] else "없음")
            
            print("\n[메뉴별 분석]")
            if result['메뉴']:
                for menu_item in result['메뉴']:
                    for menu_name, analysis in menu_item.items():
                        print(f"\n{menu_name}:")
                        print("긍정:", ", ".join(analysis['긍정']) if analysis['긍정'] else "없음")
                        print("부정:", ", ".join(analysis['부정']) if analysis['부정'] else "없음")
            else:
                print("메뉴 평가 없음")
        ...
        # 발견된 새로운 음식 출력
        unknown_foods = analyzer.get_unknown_foods()
        if unknown_foods:
            print("\n발견된 DB에 없는 음식들:")
            for food in sorted(unknown_foods):
                print(f"- {food}")
